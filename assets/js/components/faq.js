...
const Faq = () => {
  const newScore = "{elo}_{after} = {elo}_{before} + K (S - E)";

  const expectedOdds = "E_{face} = \\dfrac{Q_{face}}{Q_{face}+Q_{heel}}";
  const expectedOddsQ = "Q_{face} = 10^{{elo}_{face}/400}";

  const multiPersonOdds = "E_{face} = \\dfrac{Q_{face}}{\\sum Q}";

  const tagTeamOdds =
    "E_{face} = \\dfrac{\\sum Q_{team}}{N_{team}\\sum Q_{all}}";
    ...
    <div className="faq">
      ...
      <React.Fragment>
        ...
        <div className="plain-text">
          <div>{oddsCalculator([[1200], [1200, 1200]])}</div>
          <InlineTex
            texContent={`All wrestlers start with an elo of $$${
              eloConsts.default_elo
            }$$. After each
          match all participants of the match will get their elo updated:`}
          />
          <div className="formula">
            <Tex texContent={newScore} />
          </div>
          <InlineTex
            texContent={`where $$K$$ is the K-factor and $$S$$ and $$E$$ are the actual and expected scores
          ...
          match with $$K=${
            eloConsts.match_weight
          }$$. The actual score ($$S$$) for each
          participant of the match is $$1$$, $$0.5$$ or $$0$$ for a win, draw or loss
          respectively. The expected score ($$E$$) for each participant is
          calculated as:`}
          />
          <div className="formula">
            <Tex texContent={expectedOdds} />
            <Tex texContent={expectedOddsQ} />
          </div>
          <InlineTex
            texContent={`This is done for each match sequentially to calculate every participant's elo to 
      ...
      <React.Fragment>
        ...
        <div className="plain-text">
          ...
          <div className="formula">
            <Tex texContent={multiPersonOdds} />
          </div>
          <InlineTex
            texContent={`With this we maintain $$\\sum E = 1$$ and the total points gained will still be equal 
      ...
      <React.Fragment>
        ...
        <div className="plain-text">
          <InlineTex
            texContent={`For tag team and handicap matches we treat each team as a
          ...
          />
          <div className="formula">
            <Tex texContent={tagTeamOdds} />
          </div>
          <InlineTex
            texContent={`where $$\\sum Q_{team}$$ and $$\\sum Q_{all}$$ are the sums of the $$Q$$ values for the 
          ...
          <br />
          <InlineTex
            texContent={`For a 1v2 handicap match where all competitors have an equal elo we have an expected score 
