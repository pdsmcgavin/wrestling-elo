defmodule WweloApi.StatsTest do
  ...
  describe "wrestlers" do
    ...
    @valid_attrs %{career_end_date: ~D[2010-04-17], career_start_date: ~D[2010-04-17], current_elo: 42, draw: 42, gender: "some gender", height: 42, losses: 42, maximum_elo: 42, minimum_elo: 42, name: "some name", weight: 42, wins: 42}
    @update_attrs %{career_end_date: ~D[2011-05-18], career_start_date: ~D[2011-05-18], current_elo: 43, draw: 43, gender: "some updated gender", height: 43, losses: 43, maximum_elo: 43, minimum_elo: 43, name: "some updated name", weight: 43, wins: 43}
    @invalid_attrs %{career_end_date: nil, career_start_date: nil, current_elo: nil, draw: nil, gender: nil, height: nil, losses: nil, maximum_elo: nil, minimum_elo: nil, name: nil, weight: nil, wins: nil}
  ...
  describe "aliases" do
    ...
    test "update_alias/2 with invalid data returns error changeset" do
      alias = alias_fixture()
      assert {:error, %Ecto.Changeset{}} = Stats.update_alias(alias, @invalid_attrs)
  ...
  describe "events" do
    ...
    @valid_attrs %{arena: "some arena", brand: "some brand", date: ~D[2010-04-17], event_type: "some event_type", location: "some location", name: "some name", promotion: "some promotion"}
    ...
    test "update_event/2 with invalid data returns error changeset" do
      event = event_fixture()
      assert {:error, %Ecto.Changeset{}} = Stats.update_event(event, @invalid_attrs)
  ...
  describe "matches" do
    ...
    @valid_attrs %{card_position: 42, event_id: 42, stipulation: "some stipulation"}
  ...
  describe "participants" do
    ...
    @valid_attrs %{elo_after: 42, match_id: 42, match_team: 42, outcome: "some outcome", wrestler_id: 42}
    @update_attrs %{elo_after: 43, match_id: 43, match_team: 43, outcome: "some updated outcome", wrestler_id: 43}
    @invalid_attrs %{elo_after: nil, match_id: nil, match_team: nil, outcome: nil, wrestler_id: nil}
    ...
    test "create_participant/1 with valid data creates a participant" do
      assert {:ok, %Participant{} = participant} = Stats.create_participant(@valid_attrs)
    ...
    test "create_participant/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Stats.create_participant(@invalid_attrs)
    end

    test "update_participant/2 with valid data updates the participant" do
      participant = participant_fixture()
      assert {:ok, participant} = Stats.update_participant(participant, @update_attrs)
    ...
    test "update_participant/2 with invalid data returns error changeset" do
      participant = participant_fixture()
      assert {:error, %Ecto.Changeset{}} = Stats.update_participant(participant, @invalid_attrs)
    ...
    test "delete_participant/1 deletes the participant" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Stats.get_participant!(participant.id) end
    end
